...
13. [OOP and Design Patterns](#13-oop-and-design-patterns)
14. [Links to Official Resources and Documentation](#14-links-to-official-resources-and-documentation)
...
86. **Apply patterns intentionally:** Use established OOP patterns (Façade, Strategy, Observer, Command, State, Iterator, Decorator, Singleton, Adapter, Flyweight, Abstract Factory, Composite, Template) only when the problem they solve is present. Never apply speculatively. Document which pattern a class implements in its class docstring (e.g., `# Pattern: Façade over Kraken API + Aladdin signals`).

87. **Document class relationships:** For non-trivial hierarchies, include an ASCII or Mermaid diagram in the module docstring. Full UML is optional; relationships must be documented somewhere readable in the repo.

88. **Prefer composition over inheritance:** Inherit only for genuine is-a relationships. Use composition and protocol-based interfaces for flexibility.

89. **asyncio hygiene:** Never call blocking I/O inside `async def` without `run_in_executor`. Prefer `asyncio.TaskGroup` (3.11+) over bare `gather` for structured concurrency. Unhandled task exceptions must always be logged.

90. **Use pathlib for all filesystem operations:** Prefer `pathlib.Path` over `os.path` for all path manipulation. Do not mix the two in the same module.

91. **Prefer structural pattern matching for complex branching:** Use `match` statements (3.10+) over long `if/elif` chains when dispatching on dict shapes, return types, or tagged unions. Annotate the matched type explicitly.

92. **Validate external data at runtime boundaries:** Any data ingested from an external API, file, or inter-process source must be validated at the point of entry using Pydantic or dataclass `__post_init__`. Static type hints alone do not catch malformed live data. This rule and Rule 46 are co-dependent.
...
## 14. Links to Official Resources and Documentation
...
| Python OOP – 4th Edition (Packt) | https://github.com/PacktPublishing/Python-Object-Oriented-Programming---4th-edition |
